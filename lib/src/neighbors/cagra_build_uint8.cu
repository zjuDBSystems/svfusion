...
#define FFANNS_INST_CAGRA_BUILD(T, IdxT)                                            \
  ...
  {                                                                                                 \
    return ffanns::neighbors::cagra::build<T, IdxT>(handle, params, dataset, index_graph, device_dataset_ref, device_graph_ref, delete_bitset, tag_to_id, host_in_edges_view, d_in_edges_view, start_id, end_id);   \
  }

FFANNS_INST_CAGRA_BUILD(uint8_t, uint32_t);
