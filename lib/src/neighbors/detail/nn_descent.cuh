...
namespace ffanns::neighbors::nn_descent::detail {
                                   ...
                                   ffanns::distance::DistanceType metric = ffanns::distance::DistanceType::L2Expanded)
{
  ...
  for (int step = 0; step < raft::ceildiv(dim, raft::warp_size()); step++) {
    ...
    if (idx < dim) {
      if (metric == ffanns::distance::DistanceType::InnerProduct) {
    ...
    d_list_sizes_old_{raft::make_device_vector<int2, size_t>(res, nrow_)}
{
  ...
  if (build_config.metric == ffanns::distance::DistanceType::L2Expanded) {
...
void GNND<Data_t, Index_t>::build(Data_t* data, const Index_t nrow, Index_t* output_graph)
{
  ...
  ffanns::spatial::knn::detail::utils::batch_load_iterator vec_batches{
           ...
           index<IdxT>& idx)
{
  ...
  auto allowed_metrics = params.metric == ffanns::distance::DistanceType::L2Expanded ||
                         params.metric == ffanns::distance::DistanceType::InnerProduct;
  RAFT_EXPECTS(allowed_metrics,
               "The metric for NN Descent should be L2Expanded, or InnerProduct");
  RAFT_EXPECTS(
    idx.metric() == params.metric,
    "The metrics set in nn_descent::index_params and nn_descent::index are inconsistent");
  RAFT_LOG_INFO("params.metric is %s",
                params.metric == ffanns::distance::DistanceType::L2Expanded
                  ? "L2Expanded"
                  : params.metric == ffanns::distance::DistanceType::InnerProduct
                      ? "InnerProduct"
                      : "Unknown");
...
} // ffanns::neighbors::nn_descent::detail