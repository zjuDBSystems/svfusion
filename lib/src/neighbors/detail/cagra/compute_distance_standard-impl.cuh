...
namespace {
template <typename T, ffanns::distance::DistanceType Metric>
RAFT_DEVICE_INLINE_FUNCTION constexpr auto dist_op(T a, T b)
  -> std::enable_if_t<Metric == ffanns::distance::DistanceType::L2Expanded, T>
{
  T diff = a - b;
  return diff * diff;
}
...
template <typename T, ffanns::distance::DistanceType Metric>
RAFT_DEVICE_INLINE_FUNCTION constexpr auto dist_op(T a, T b)
  -> std::enable_if_t<Metric == ffanns::distance::DistanceType::InnerProduct, T>
{
  return -a * b;
}
}  // namespace
...
struct standard_dataset_descriptor_t : public dataset_descriptor_base_t<DataT, IndexT, DistanceT> {
  ...
  static constexpr RAFT_INLINE_FUNCTION auto dd_ptr(const args_t& args) noexcept
    -> const DATA_T* const&
  {
    return (const DATA_T* const&)(args.extra_ptr2);
  }
  static constexpr RAFT_INLINE_FUNCTION auto dd_ptr(args_t& args) noexcept -> const DATA_T*&
  {
    return (const DATA_T*&)(args.extra_ptr2);
  }
                ...
                get_smem_ws_size_in_bytes(dim))
  {
    ...
    standard_dataset_descriptor_t::dd_ptr(args) = dd_ptr;
  ...
  uint32_t query_id) -> const DescriptorT*
{
  ...
  for (unsigned i = threadIdx.x; i < buf_len; i += blockDim.x) {
    ...
    if (i < dim) {
      buf[j] = ffanns::spatial::knn::detail::utils::mapping<QUERY_T>{}(queries_ptr[i]);
    } else {
      buf[j] = 0.0;
    }
  }
...
#pragma unroll
      for (uint32_t v = 0; v < vlen; v++) {
        ...
        r += dist_op<DISTANCE_T, DescriptorT::kMetric>(
          d, ffanns::spatial::knn::detail::utils::mapping<DISTANCE_T>{}(data[e][v]));
      }
    }
  }
  return r;
  ...
  typename DescriptorT::DISTANCE_T
{
  return compute_distance_standard_worker<DescriptorT>(
    DescriptorT::dd_ptr(args) + (static_cast<std::uint64_t>(DescriptorT::ld(args)) * dataset_index),
  ...
  const cagra::search_params& params, const DataT* ptr, const DataT* dd_ptr, IndexT size, uint32_t dim, uint32_t ld)
{
  using desc_type =
    standard_dataset_descriptor_t<Metric, TeamSize, DatasetBlockDim, DataT, IndexT, DistanceT>;
  using base_type = typename desc_type::base_type;
  desc_type dd_host{nullptr, nullptr, ptr, dd_ptr, size, dim, ld};
  return host_type{dd_host,
                   [=](dataset_descriptor_base_t<DataT, IndexT, DistanceT>* dev_ptr,
                     ...
                     standard_dataset_descriptor_init_kernel<Metric,
                                                             ...
                                                             IndexT,
                                                             DistanceT>
                       <<<1, 1, 0, stream>>>(dev_ptr, ptr, dd_ptr, size, dim, ld);
