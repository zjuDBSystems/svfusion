...
for type_path, (data_t, idx_t, distance_t) in search_types.items():
    ...
    with open(path, "w") as f:
        f.write(header)
        f.write(
                f"instantiate_kernel_selection(\n  {data_t}, {idx_t}, {distance_t}, ffanns::neighbors::filtering::none_sample_filter);\n"
        )
        # f.write(
        #         f"instantiate_kernel_selection(\n  {data_t}, {idx_t}, {distance_t}, CagraSampleFilterWithQueryIdOffset<ffanns::neighbors::filtering::bitset_filter<uint32_t COMMA int64_t>>);\n"
        # )
