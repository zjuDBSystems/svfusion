...
inline std::enable_if_t<std::is_floating_point_v<MathT>> predict_core(
  ...
  const ffanns::cluster::kmeans::balanced_params& params,
  const MathT* centers,
  IdxT n_clusters,
  IdxT dim,
  const MathT* dataset,
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  switch (params.metric) {
    ...
    case ffanns::distance::DistanceType::L2SqrtExpanded: {
      ...
      ffanns::distance::fusedDistanceNNMinReduce<MathT, raft::KeyValuePair<IdxT, MathT>, IdxT>(
        minClusterAndDistance.data_handle(),
        dataset,
        centers,
        dataset_norm,
        ...
        (params.metric == ffanns::distance::DistanceType::L2Expanded) ? false : true,
        false,
        true,
        params.metric,
        0.0f,
    ...
    case ffanns::distance::DistanceType::CosineExpanded: {
      auto workspace = raft::make_device_mdarray<char, IdxT>(
        handle, mr, raft::make_extents<IdxT>((sizeof(int)) * n_rows));
                            ...
                            rmm::device_async_resource_ref mr)
{
  ...
  ffanns::cluster::kmeans::detail::countLabels(
             ...
             const MathT* dataset_norm                        = nullptr)
{
  ...
  bool need_compute_norm =
    dataset_norm == nullptr && (params.metric == ffanns::distance::DistanceType::L2Expanded ||
                                params.metric == ffanns::distance::DistanceType::L2SqrtExpanded ||
                                params.metric == ffanns::distance::DistanceType::CosineExpanded);
  ...
  for (IdxT offset = 0; offset < n_rows; offset += max_minibatch_size) {
    ...
    if (need_compute_norm) {
      if (params.metric == ffanns::distance::DistanceType::CosineExpanded)
        compute_norm(handle,
                        ...
                        rmm::device_async_resource_ref device_memory)
{
  ...
  for (uint32_t iter = 0; iter < n_iters; iter++) {
    ...
    switch (params.metric) {
      ...
      case ffanns::distance::DistanceType::InnerProduct:
      case ffanns::distance::DistanceType::CosineExpanded:
      case ffanns::distance::DistanceType::CorrelationExpanded: {
                         ...
                         rmm::device_async_resource_ref device_memory) -> IdxT
{
  ...
  for (IdxT i = 0; i < n_mesoclusters; i++) {
    ...
    if (params.metric == ffanns::distance::DistanceType::L2Expanded ||
        params.metric == ffanns::distance::DistanceType::L2SqrtExpanded ||
        params.metric == ffanns::distance::DistanceType::CosineExpanded) {
                        ...
                        const MathT* dataset_norm = nullptr)
{
  ...
  if (dataset_norm == nullptr && (params.metric == ffanns::distance::DistanceType::L2Expanded ||
                                  params.metric == ffanns::distance::DistanceType::L2SqrtExpanded ||
                                  params.metric == ffanns::distance::DistanceType::CosineExpanded)) {
